...
class MLP():
	...
	def run_epoch(self):
		...
		for entry in self.train_data:
			...
			new_hidden_to_output = np.outer(first_hidden_z_values, delta_outputs)*self.step_size+self.weight_matrices[1]
			self.weight_matrices[1] = new_hidden_to_output
			new_input_to_hidden = np.outer(entry[0], first_hidden_deltas)*self.step_size+self.weight_matrices[0]
	...
	def fit(self, input_values, input_labels):
		...
		for entry in input_labels:
			label_matrix.append(label_to_matrix_dictionary[entry])
